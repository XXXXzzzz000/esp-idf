...
/* FreeRTOS事件组用于在准备好或连接时发送信号以进行应答*/
static EventGroupHandle_t wifi_event_group;

/* 事件组允许每个事件有多个位，但我们只关心一个事件 - 我们是否使用IP连接到AP？ */
...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
        /* 这是一个解决方法，因为ESP32 WiFi库当前不会自动重新关联。 */
        got_ip_flag = 0;
        esp_wifi_connect();
...
static void initialise_wifi(void)
{
    ...
    ESP_LOGI(TAG, "Setting WiFi configuration ssid: %s password: %s",
...
#define SERVER_IP "39.108.223.153"
