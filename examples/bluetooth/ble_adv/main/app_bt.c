...
void app_main()
{
     ...
     *
     * 如果在这里调用mem释放，也可以工作。 输入ESP_BT_MODE_CLASSIC_BT，该功能将释放经典BT模式的内存。
     * esp_bt_controller_mem_release（ESP_BT_MODE_CLASSIC_BT）;
     */
     ...
     * 
     * 如果在这里调用mem释放，也可以工作。 输入ESP_BT_MODE_BTDM，函数会计算出BLE模式已经被使用，所以它只会释放经典的bt模式。
     * esp_bt_controller_mem_release（ESP_BT_MODE_BTDM）;
     */
    /* TODO:创建一个线程? */
