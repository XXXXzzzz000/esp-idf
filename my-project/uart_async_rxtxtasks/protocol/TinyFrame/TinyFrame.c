...
{
    //TODO:用完删除
    // ESP_LOGE("TF", "TF_HandleReceivedMessage()");
    TF_COUNT i;
    ...
    for (i = 0; i < tf->count_generic_lst; i++) {
        ...
        if (glst->fn) {
            res = glst->fn(tf, &msg);
            //TODO:用完删除
            // ESP_LOGE("TF", "count_generic_lst[%d]", i);
            if (res != TF_NEXT) {
...
void _TF_FN TF_Accept(TinyFrame *tf, const uint8_t *buffer, uint32_t count)
{
    ...
    // //TODO:
    // ESP_LOGI("RX_TASK", "Read %d bytes: '%s'", count, buffer);
    // ESP_LOG_BUFFER_HEXDUMP("RX_TASK", buffer, count, ESP_LOG_INFO);
}
...
{
    //TODO:
    // ESP_LOGE("TF", "TF_AcceptChar(),c=%x", c);
